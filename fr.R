...
! [*** Figure 1: ** ** A) ** scores ISO canadiens (CISO) (Daigle et al., Données non publiées), pour les indicateurs individuels ou sous-produits (pétales colorés) et global (nombre central) pour 2013. Dans CISO, les indicateurs sont pondérés en fonction d'un modèle discret discret de classe latente. Les chiffres en italique montrent le statut actuel de l'indicateur. Les sous-produits de la mariculture (MAR) et des pêches (FIS) font partie de l'indicateur de la fourniture de nourriture; Besoins autochtones (AN); Produits naturels (NP); Stockage au carbone (CS); Protection côtière (CP); Tourisme et loisirs (TR); Les sous-gardes des moyens de subsistance (LIV) et des économies (ECO) font partie de l'indicateur des moyens de subsistance côtiers; Les ingrédients iconiques (ICO) et les sous-objets LSP (Lasting Special Places) font partie de Sense of Place; Eaux propres (CW); Les sous-gardes de l'Habitat (HAB) et de la Protection des espèces (SPP) font partie de l'indicateur de la Biodiversité. ** B) ** Importance relative de 10 avantages dérivés des océans pour les Canadiens, mesurés en tant que poids d'une analyse de classe latente d'une classe. Les moyens sont affichés ± 1 SD. Nombre total de répondants = 2000. Adapté de [Daigle et al. 2016] (http://www.sciencedirect.com/science/article/pii/S0308597X16303323) *] (COHI.png)
